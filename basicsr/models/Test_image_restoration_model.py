...
class TestImageRestorationModel(BaseModel):
    """Base Deblur model for single image deblur."""

    def __init__(self, opt):
        super(TestImageRestorationModel, self).__init__(opt)
    ...
    # def grids_inverse_voxel(self):
    #     preds = torch.zeros(self.original_size_voxel).to(self.device)
    #     b, c, h, w = self.original_size_voxel
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            while i < n:
                ...
                pred = self.net_g(x = self.lq[i:j, :, :, :])  # mini batch all in 
